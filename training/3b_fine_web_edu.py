#!/usr/bin/env python3
"""
BushidoMythos pretraining on FineWeb-Edu with FSDP + AdamW.
...
from bushido_mythos.main import TransformerBlock, RecurrentBlock
from bushido_mythos.variants import mythos_3b
    ...
    try:
        ...
        if not allow_unsafe:
            raise RuntimeError(
                ...
                "Re-run with allow_unsafe=True to permit pickle-based loading."
            ) from first_err
        import warnings
        warnings.warn(
            f"weights_only=True failed for {path!r}: {first_err}. Falling back to weights_only=False.",
            stacklevel=2,
        )
...
def load_checkpoint(model, optimizer, path: str, ddp: bool) -> int:
    ...
    Uses `_safe_torch_load` which defaults to `weights_only=True` for
    security; falls back to pickle-based loading only if `allow_unsafe=True`
...
def main():
    ...
    Lifecycle:
        ...
        3. Construct BushidoMythos with the 3B variant config.
    ...
    while step < total_steps:
        ...
        raw_model.update_moe_router_bias(distributed=ddp)
        step += 1
