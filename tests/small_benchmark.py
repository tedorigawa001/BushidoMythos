#!/usr/bin/env python3
"""
Side-by-side training + benchmark of BushidoMythos vs. a vanilla transformer on a
...
3. Depth-extrapolation sweep at the end: BushidoMythos is trained at
...
class BaselineTransformer(nn.Module):
    ...
    Reuses BushidoMythos's TransformerBlock (attention + FFN kernels are identical)
...
def evaluate(
    ...
    `n_loops` is only forwarded to BushidoMythos; for any other module the kwarg
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--depth-sweep",
        default="1,2,4,8,16",
        help="comma-separated n_loops values for BushidoMythos depth-extrapolation eval",
    )
...
def main() -> None:
    ...
    torch.manual_seed(args.seed)
    mythos = BushidoMythos(cfg).to(device)
    ...
    print(
        f"[setup] BushidoMythos params  = {fmt_count(n_m)}  ({n_m:,})\n"
    ...
    print(f"  {'':<24} {'BushidoMythos':>16}   {'Baseline':>16}")
    ...
    # ------------------------------------------------------------------
    # Depth extrapolation: BushidoMythos eval loss as a function of n_loops.
    ...
    print(f"  BushidoMythos (trained at n_loops={cfg.max_loop_iters}):")
