"""
BushidoMythos vs. vanilla GQA+MoE transformer benchmark.

Compares BushidoMythos (Prelude + looped Recurrent Block + Coda with ACT halting,
...
non-recurrently. The baseline reuses BushidoMythos primitives so the comparison
...
Metrics reported:
    ...
    - BushidoMythos depth-scaling sweep: latency vs. n_loops
...
from bushido_mythos import MythosConfig, BushidoMythos, mythos_1b
...
class BaselineTransformer(nn.Module):
    ...
    with BushidoMythos so any speed delta is attributable to the recurrent-depth
...
def bench_prefill(
    ...
    if isinstance(model, BushidoMythos):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--n-loops",
        default="1,4,8",
        help="comma-separated loop counts to sweep (BushidoMythos only)",
    )
    p.add_argument(
        "--decode-steps",
        type=int,
...
def main() -> None:
    ...
    # to match the unique-parameter depth of BushidoMythos (parameter-matched baseline).
    baseline_n_layers = cfg.prelude_layers + 1 + cfg.coda_layers

    torch.manual_seed(0)
    mythos = BushidoMythos(cfg).to(device=device, dtype=dtype).eval()
    torch.manual_seed(0)
    baseline = (
    ...
    print(
        f"  BushidoMythos : total={fmt_count(m_params.total):>10}   "
    ...
    for seq_len in seq_lens:
        ...
        for nl in n_loops_sweep:
            ...
            mem = peak_mem_mb(device)
            print(
                f"  {'BushidoMythos (loops=' + str(nl) + ')':<26} {seq_len:>6} "
                f"{secs*1000:>9.2f}ms {tps:>12,.0f} {mem:>10.1f}"
            )
    ...
    for nl in n_loops_sweep:
        ...
        print(
            f"  {'BushidoMythos (loops=' + str(nl) + ')':<26} "
            f"{per_step*1000:>10.2f}ms {tps:>14,.1f}"
        )

    # ---- Depth scaling ----
    print_header(
        "BushidoMythos depth scaling (fixed seq={}, batch={})".format(
